...
bool SndO::Open (const char *name, mixfunc pmix, ubyt4 frq, ubyt4 nFr)
...
{DBG("pw_main_loop_new DIED !!");   return false;}
  ...
  struct pw_properties *props = pw_properties_new (
   ...
   if (! _pwStream)
{DBG("pw_stream_new_simple DIED");   return false;}
...
// ok!  got our pwBuf > pl(ugin) data > buf
  const ubyt4 szFr = sizeof (real4) * 2;    // size of frame (real4 stereo)
   ...
   if (nFr > 0) {
   // if paused, just zero it else we call _mix to fill it
