...
typedef struct {             // stereo wavs get split to 2 samples
   ...
   ulong pos, lpBgn, len;    // where it's at in AudO.Smp[] in samples
...
public:
   ...
   ulong _siz,  _mxDat;      // #samples of all WAVs, max DAT buf in bytes needd
   ...
   ulong LoadDat (ubyte *dat, ulong pos)
   ...
   { TStr  fn;
     ...
     real  sr;
     bool  got;
     struct {char tag [4];  ulong siz;} ch;
     char          id [4];
...
//TRC("i=`d/`d fn=...\\`s'", i, _nSmp, _smp [i].fn);
         ...
         while (f.Get (& ch, sizeof (ch)) == sizeof (ch)) {
            if (! MemCm (ch.tag, "data", 4, 'x'))
                {lenD = ch.siz;   if (f.Get (dat, lenD) == lenD)  got = true;}
            else f.Seek (EVEN_UP (ch.siz), ".");      // skip other Chunks
         }
         f.Shut ();
         if (! got)  Die ("Sound::LoadDat  no WAV data chunk", fn);
//TRC("lenD=`d", lenD);
...
//______________________________________________________________________________
   ...
   { TStr  fn;
     ...
     ubyte bt, ch;
     real  fr;
     struct {char tag [4];  ulong siz;} chk;
     char          id [4], got [3];
     WAVEFORMATEXTENSIBLE  wf;
     struct {
        ulong manuf;  ulong prod;  ulong per;  ulong note;  ulong frac;
     ...
     Sample *s = & TSmp [_nSmp];
      ...
      while (f.Get (& chk, sizeof (chk)) == sizeof (chk)) {
         if      (! MemCm (chk.tag, "fmt ", 4, 'x')) {
         ...
         else if (! MemCm (chk.tag, "data", 4, 'x')) {
            got [1] = 'y';
            lenD = chk.siz;
            f.Seek (EVEN_UP (chk.siz), ".");     // skip fer now
         }
         else if (! MemCm (chk.tag, "smpl", 4, 'x')) {
            got [2] = 'y';
            if (chk.siz > sizeof(smpl)) {
               if (f.Get (& smpl, sizeof(smpl)) != sizeof(smpl))
                  {DBG ("Sound::LoadFmt  eof1 in smpl `s", fn); return false;}
               f.Seek (EVEN_UP(chk.siz - sizeof(smpl)), ".");
            }
            else
            ...
            f.Seek (EVEN_UP (chk.siz), ".");     // skip unused Chunks
      }
      f.Shut ();
...
//DBG("loop off for `s (len=`d lpBgn=`d lpEnd=`d)", fn,ln,smpl.bgn,smpl.end);
      ...
      s->cnt = (ubyte)((ulong)smpl.frac / ((ulong)0x80000000/50));
...
TRC("SndDir bgn _nm=`s dss=`s dds=`s", snd, dss, dds);
      StrCp (_nm, snd);   App.Path (_pa, 'd');   StrAp (_pa, "\\device\\syn\\");
...
DBG("no \\ in `s", _nm);
      ...
      if (i > 5) {
         if ((! MemCm (_nm, "Drum\\", 5)) ||
             (! MemCm (& _pa [i-5], "_drum", 5))) _xFrq = true;
         if  (! MemCm (& _pa [i-5], "_hold", 5))  _xRls = true;
         if  (! MemCm (& _pa [i-5], "_clip", 5))  _xFrq = _xRls = true;
      }
   // if default sampset, look for it with any _morename
      if ( (! f.PathGot (_pa)) && (! StrCm (ss, dss)) ) {
TRC(" resolving path=`s for GM", _pa);
         StrCp (dr, _pa);   Fn2Path (dr);   // dr has par dir
         StrCp (_pa, & _pa [StrLn (dr)+1]); // _pa has just fn prefix
         i = d.DLst (dr, dfd, (uword)BITS (dfd));
...
TRC("SndDir end _pa=`s xFrq=`b xRls=`b", & _pa [ig], _xFrq, _xRls);
   ...
   Sound (char *snd, ubyte dKey = 128, char *dss = "", char *dds = "")
...
DBG("Sound::SndDir couldn't turn `s into a dir :(", snd);
   ...
   // _v<velo>_ in fn gives optional max velocity range
      nw = d.FLst (_pa, WavFn, (uword)BITS (WavFn), "*.wav");
      StrAp (_pa, "\\");
